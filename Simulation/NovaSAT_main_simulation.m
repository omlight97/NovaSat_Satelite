...
%         end
    ...
    %end
    % if not(Flags.Communication) &&(Flags.Day) && not(Flags.GRB)
    % Next_Step_Angular.Theta=0;
    % end
...
% plot(Time_vec/60,100*DayOrNight_vec);
% bar(Time_vec/60,10*AccessSumTimeGS(:,1));
% plot(Time_vec/60,100*GRB_Alert);
% legend('total power','day mode','communication mode','grb mode');
xlabel('time [min]');
...
xlabel('time [min]');
ylabel('Batteries ElectricCharge [Wh]');
title('Batteries ElectricCharge vs time');
grid on;
subplot(2,1,2)
hold on;
plot(Time_vec/60,100.*DOD_vec);
yline(100*DOD_avg,'g','LineWidth',1.5);
xlabel('time [min]');
ylabel('%');
title('DOD vs time');
grid on;
...
figure()
% sgtitle(['Orbit For a=', num2str(a(1)), ' [km], i=', num2str(i(1)), '[deg]'])
beta=table2array(DataBase.SatProperties(:,1));
plot(Time_vec/60,beta);
xlabel('time [min]');
ylabel('beta [KM]');
title('beta(t)');
grid on;
