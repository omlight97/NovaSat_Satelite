...
[T, a1, P, rho] = atmosisa(r_sat(i));
    rho2(i)=rho;
    q(i)=0.5*rho*v_sat(i)^2;
    alpha_G_0=10;
    n=6;
...
subplot(1,3,1);
hold on;
plot(Time_vec(1:1240)/60,SimData.Power.Total_Power');
xlabel('time [min]');
...
subplot(1,3,2);
hold on;
plot(Time_vec(1:1240)/60,SimData.Power.Production');
xlabel('time [min]');
ylabel('Production [w]');
...
subplot(1,3,3);
hold on;
plot(Time_vec(1:1240)/60,SimData.Batteries_Electric_Charge');
xlabel('time [min]');
